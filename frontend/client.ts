...
export const Local: BaseURL = "http://10.0.2.2:4000"

/**
 * Environment returns a BaseURL for calling the cloud environment with the given name.
 */
export function Environment(name: string): BaseURL {
    return `https://${name}-e-commerce-platform-ggci.encr.app`
}
...
/**
 * Client is an API client for the e-commerce-platform-ggci Encore application.
 */
export class Client {
...
import { search as api_order_search_search } from "~backend/order/search";
...
export namespace order {
    ...
    export class ServiceClient {
        ...
        public async search(params: { orderNumber: string }): Promise<ResponseType<typeof api_order_search_search>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/orders/search/${encodeURIComponent(params.orderNumber)}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_order_search_search>
        }
    }
}
...
import { remove as api_product_remove_remove } from "~backend/product/remove";
